...
async createRental(req, res) {
  try {
    ...
    await new Notification({
        ...
        text: `Пользователь ${user.name} хочет арендовать ваш автомобиль "${car.name}"`,
        linkTo: `/rental/${rental._id}`
    }).save();
  ...
  try {
    ...
    await new Notification({
        user: rental.user,
        title: `Статус аренды изменён на "${status}"`,
        text: status === 'paid'
            ? "Продавец подтвердил оплату наличными."
